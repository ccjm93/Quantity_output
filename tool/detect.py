...
- overflow: 숫자 셀이 열 폭 부족으로 '###'(또는 '#######')로 표시됨 → Range.Text 가 '#' 로만 채워짐.
- error:    수식 오류 셀(#REF!, #DIV/0!, #VALUE!, #NAME?, #N/A, #NUM!, #NULL!).
...
# '#' 로만(공백 허용) 이루어진 표시 = 열 폭 부족 오버플로우
_HASH_ONLY = re.compile(r"^#+\s*$")
...
def _iter_cells(rng):
    ...
    try:
        ...
        return
    for ai in range(1, areas.Count + 1):
        try:
            area = areas.Item(ai)
            cells = area.Cells
            n = cells.Count
...
def scan_sheet_issues(ws) -> list:
    """Returns [{sheet, cell, kind, text}]  (kind: 'overflow' | 'error').

    주의: 이 탐지는 셀의 '표시 텍스트(Range.Text)'가 ###/오류일 때만 잡는다.
    그림/단면도 안의 숫자가 '출력 배율' 때문에 ###로 찌그러지는 경우는 셀 값 자체가
    정상이라 여기서 잡히지 않는다(그건 출력 설정 문제이며 시각/AI 검토 영역).
    """
    ...
    # 1) 오버플로우(###): 숫자 상수 + 숫자 수식 결과 셀만 검사
    for ctype in (XL_CELLTYPE_CONSTANTS, XL_CELLTYPE_FORMULAS):
        rng = _special(ws, ctype, XL_NUMBERS)
        if rng is None:
            continue
